...
(() => {
  ...
  function boot(runtime, doc = document) {
    ...
    // Ultrarealism 3D Parallax Tilt
    doc.addEventListener("mousemove", (e) => {
      const rx = (e.clientX / window.innerWidth - 0.5) * 2;
      const ry = (e.clientY / window.innerHeight - 0.5) * 2;
      hostEl.style.transform = `perspective(1500px) rotateX(${-ry * 1.5}deg) rotateY(${rx * 1.5}deg)`;
      hostEl.style.transformStyle = "preserve-3d";
    });
    // Smooth return to center when mouse leaves
    doc.addEventListener("mouseleave", () => {
      hostEl.style.transform = `perspective(1500px) rotateX(0deg) rotateY(0deg)`;
      hostEl.style.transition = "transform 0.5s ease-out";
    });
